...
def get_shaded_data(df_all, var, ens, freq='Monthly'):
    ...
    df_out2 = pd.DataFrame({'time':df['time'],'year':df['year'],'month': df['month'], 'var':this})
...
def shaded_tseries(doc):
    ...
    vars_dict = {'TREFHTMN': 'Minimum Temperature', 'TREFHTMX': 'Maximum Temperature',\
     'PRECT': 'Total  Precipitation','SOILWATER_10CM': 'Soil Moisture (Top 10 cm) '}
    ...
    menu = Select(options=list(vars_dict2.keys()),value=vars_dict[defualt_var], title='Variable', css_classes=['custom_select']) 
    ...
    menu_freq = Select(options=freq_list,value=default_freq, title='Frequency', css_classes=['custom_select']) 
